...
Artificial models process data very differently from biological neural cells. If we train a deep network to predict a brain wave, we might just be doing high dimensional curve fitting (based on different reward mechanism, models now how to mimic a line dance). We are essentially using one black box to explain another black box. Even if the prediction accuracy is high, it does not mean the model has learned the actual cognitive mechanism behind the scenes. Without revealing a clear biological mechanism, this technology remains a statistical trick rather than a true scientific insight.
